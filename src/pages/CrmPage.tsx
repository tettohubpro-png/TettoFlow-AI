...
export function CrmPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    setActionError(null)
    const { data, error: err } = await createClient({ name, status: 'ACTIVE' })
    if (err) {
      setActionError(err)
      return
    }
    if (runAfterCreate && data?.id) {
    ...
    <div>
      ...
      {showForm && (
        <form
          ...
          className="mb-6 space-y-3 rounded-xl border border-slate-800 bg-slate-900/50 p-4 sm:p-5"
        >
          <input
            ...
            className="min-h-11 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          />
          <label className="flex min-h-11 items-center gap-2 text-sm text-slate-400">
            <input
              type="checkbox"
          ...
          <button
            type="submit"
            disabled={running}
            className="min-h-11 w-full rounded-lg bg-emerald-600 px-6 py-2.5 font-medium disabled:opacity-50 sm:w-auto"
          >
            {running ? 'Processando...' : 'Salvar'}
          </button>
        </form>
      )}
